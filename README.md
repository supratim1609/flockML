...
1. **8-Bit Quantization (`src/quantization.ts`)**
...
2. **Differential Privacy (`src/privacy.ts`)**
...
3. **Federated Averaging Coordinator (`src/coordinator.ts`)**
...
import { SwarmNode } from 'flockml';

// 2. Connect to your central FedAvg Coordinator
SwarmNode.connect('wss://api.yourdomain.com/flockml');

// 3. Initialize background training
SwarmNode.startTraining();
```
